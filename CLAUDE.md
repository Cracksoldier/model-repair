...
- **Vulkan GPU smoothing**: `MODELREPAIR_ENABLE_VULKAN=ON` (default) enables GPU-accelerated smoothing. Requires `vulkan-headers`, `vulkan-icd-loader`, and `glslc` (from `shaderc`). When Vulkan + glslc are both found, `MODELREPAIR_HAVE_VULKAN` is defined, `VulkanSmoothing.cpp` is compiled, and the "Use GPU (Vulkan)" checkbox is enabled at runtime if a Vulkan device is detected. The GLSL shader `src/shaders/smooth_laplacian.comp` is compiled to SPIR-V at build time via `glslc`, then embedded as a `uint32_t[]` header (`smooth_laplacian_spv.h`) via `cmake/EmbedSpv.cmake`. Key approximation: cotangent weights and crease falloffs are precomputed once from the initial mesh (not updated per-iteration) and uploaded as a CSR sparse matrix — this avoids all CPU↔GPU round-trips between iterations. The "Use GPU (Vulkan)" checkbox is only enabled in the GUI when smooth is checked AND Vulkan is available (`smooth_vulkan_available()` returns true; result is cached).
