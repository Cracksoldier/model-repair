...
- **`normal_to_displacement(const std::string& path, const NormalToDisplacementSettings&, std::function<bool(int)> on_iteration = nullptr)`** (`include/modelrepair/NormalToDisplacement.hpp`, `src/analysis/NormalToDisplacement.cpp`) — free function; pure image-in / image-out Poisson reconstruction of a height map from a tangent-space normal map. No mesh required. Steps: (1) load with `stb_image` (`STB_IMAGE_IMPLEMENTATION` + `STB_IMAGE_STATIC` defined in this TU); raw buffer is freed via RAII (`StbiDeleter` unique_ptr) so it is released even if subsequent Eigen allocation throws; (2) decode tangent normals (optional `flip_green`), normalize, clamp `nz` to 1e-4 to avoid division by zero, build gradient field `gx/gy = -nx_v/nz_v * gradient_strength` clamped to ±20; (3) compute backward-difference divergence `div = dgx/dx + dgy/dy`, negate to form the right-hand side; (4) build an `Eigen::SparseMatrix<float>` with a 5-point Laplacian using Neumann reflection at image edges and Dirichlet BC at pixel 0 (row 0 = identity, `b[0] = 0` to remove the constant-mode singularity; the `add_off` lambda always increments the diagonal count before the Dirichlet skip so the true graph degree is preserved); (5) solve with a manual Preconditioned CG (PCG) loop using `Eigen::IncompleteCholesky<float, Eigen::Lower>` as the preconditioner (`solver_max_iter` default 300, tolerance 1e-5 relative); IC factorisation is checked (`ic.info() != Eigen::Success` throws); flat images (b=0) are detected via `tol_sq == 0` and return a zero height map immediately without entering the solver; the search direction `Ap` is pre-allocated outside the loop and assigned with `.noalias()` to avoid per-iteration heap allocation; `on_iteration(iter_1based)` is called after each completed CG step and returns false to cancel cooperatively; IC dramatically reduces the effective condition number vs. Jacobi — convergence in 50–150 iterations instead of ~3000 for a 1024×1024 image; (6) optional box blur, normalize to [0,1], apply contrast, optionally invert. Hard limit: 8,000,000 pixels (≈2828×2828); throws with a clear message if exceeded. Returns `NormalToDisplacementResult{height_vec, width, height_px, duration_ms, solver_iterations, solver_converged}`. Note: in the debug+ASan build each PCG iteration is ~10–50× slower than release due to cache-miss amplification from ASan shadow memory; for production work use the release build. Used by `NormalToDisplacementWindow` in the GUI's **Tools** dropdown.
...
`NormalToDisplacementWindow` (`gui/NormalToDisplacementWindow.hpp/cpp`) is a non-modal `QWidget` (singleton-tracked in `MainWindow` via `nm_disp_win_`, same pattern as `wizard_`) opened from the **Tools** dropdown toolbar. Left panel: image path + Browse, settings (flip green, invert, gradient strength, contrast, blur radius, solver iterations, normalize), Run + Cancel + Export buttons, a determinate progress bar (0 → `solver_max_iter`), a live elapsed/ETA label, status label, and a small info label explaining the 8 MP limit and iteration guidance. Right panel: `QLabel` preview showing the result as a grayscale 8-bit `QPixmap` scaled to fit; `resizeEvent` is overridden to re-call `update_preview()` so the pixmap rescales when the window is resized. Run uses the Qt 6 QPromise-based `QtConcurrent::run` overload: the worker lambda calls `promise.setProgressValue(iter)` after each PCG iteration, which fires `progressValueChanged` on the main thread (connected to `on_progress`); `promise.isCanceled()` is checked inside the `on_iteration` callback so `watcher_->cancel()` (called from `on_cancel_clicked`) interrupts the solver within one iteration. `on_progress(int iter)` updates the progress bar, stops the 1-second fallback timer on the first real iteration, and shows `"elapsed / ~ETA"` using `gui::fmt_elapsed()` from `timer_util.hpp`; ETA is computed as `elapsed * (max - iter) / iter`. `on_result_ready` checks `watcher_->future().isCanceled()` first; cancellation and error paths both call `lbl_elapsed_->clear()`. On success, `set_running(false)` enables Export (via `!result_.height.empty()`), so Export state is always derived from the current result. Export saves a 16-bit PNG using `QImage::Format_Grayscale16` and `reinterpret_cast<quint16*>(img.scanLine(y))`.
